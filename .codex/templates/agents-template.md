...
This template guides the creation of a new `AGENTS.md` file. It is written for an LLM that must infer and invent a suitable roster of Codex-compatible agents for the project. Use information from `.codex/PRD.md`, `.codex/ARCHITECTURE.md` to craft concise, well defined agent profiles. The output should read like a professional brief generated by an experienced AI engineer. You should generate a file in the exact format as you see below, tailored specifically to the users PRD and ARCHITECTURE data.

Do NOT make any changes to the following agents: [`doc_writer`, `task_logger`, `feedback_interpreter`, `planner`].

# AGENTS.md

A list of Codex-compatible agents for the <project name> project. Each agent includes a clear name, defined role, core goal, and a short backstory to establish its behavior and purpose.
